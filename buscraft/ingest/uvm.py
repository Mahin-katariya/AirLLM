...
_UVM_SEV = re.compile(
    r"UVM_(?P<sev>FATAL|ERROR|WARNING|INFO)\s+"
    r"(?:@\s*(?P<time>[^\s:]+)\s*:\s*)?"
    r"(?:\[(?P<comp>[^\]]+)\]\s*)?"
    r"(?P<rest>.*)",
    re.IGNORECASE,
...
# Alternative: "UVM_ERROR /path/file.sv(12) @ 100 ns" styles
_UVM_ALT = re.compile(
    r"UVM_(?P<sev>FATAL|ERROR|WARNING|INFO)\s+"
    r"(?P<path>[^\s]+\.sv\(\d+\))?\s*"
    r"(?:@\s*(?P<time>[^\s:]+)\s*)?"
...
def _parse_time_to_ps(time_str: str | None) -> int | None:
    ...
    if t.endswith("ps"):
        ...
        mult, t = 1 / 1000, t[:-2]  # rare
    elif t.endswith("s") and not t.endswith("ns"):
        mult, t = int(1e12), t[:-1]
    try:
